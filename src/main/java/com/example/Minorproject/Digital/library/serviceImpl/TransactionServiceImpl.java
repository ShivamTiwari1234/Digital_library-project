...
import com.example.Minorproject.Digital.library.Exception.TransactionServiceException;
...
import com.example.Minorproject.Digital.library.repository.TransactionRepositoryInterf;
...
//        6) Make the Book Available
        ...
        if (transactionType.equalsIgnoreCase(TransactionType.ISSUE.toString())) {
            ...
            Transaction transaction = Transaction.builder()
                    .externalId(UUID.randomUUID().toString()).transactionType(TransactionType.ISSUE)
                    .payment(book.get().getCost()).book(book.get()).student(student).build();
